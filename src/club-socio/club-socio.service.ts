...
export class ClubSocioService {
    ...
    async findMemberFromClub(clubId: string, socioId: string): Promise<SocioEntity> {
      ...
      if (!club)
        ...
        if (!clubSocio)
          throw new BusinessLogicException("The club/member with the given id was not found",BusinessError.PRECONDITION_FAILED);
    ...
    async updateMembersFromClub(clubId: string, socios: SocioEntity[]): Promise<ClubEntity> {
      ...
      for (let i = 0; i < socios.length; i++) {
        ...
        if (!socio)
          throw new BusinessLogicException("The member with the given id is not associated to the club",BusinessError.NOT_FOUND);
      }
